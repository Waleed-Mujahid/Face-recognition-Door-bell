...
model with the following command:
  ...
  **_NOTE:_**converter = tf.lite.TFLiteConverter.from_keras_model(model)
  **_NOTE:_**tflite_model = converter.convert()

7. Now convert this model into a C++ array so it can be 
read by our micro-controller.

  **_NOTE:_**!apt-get -qq install xxd        (Get the xxd command) 
  **_NOTE:_**!xxd -i model.tflite > model.cc
