...
#define CHANNELS 1
#define DEST_WIDTH 96              // Model expects 96 x 96 input 
#define DEST_HEIGHT 96  
...
float threshold = 0.7;                        // Setting the threshold for the model according to requirements
...
constexpr int kTensorArenaSize = 75  * 1024;             // An area of memory to use for input, output, and intermediate arrays.
...
void loop() {
  ...
  downscaler.downscale(frame->buf, resized);          // Downscales the image to 96 x 96 so it can be fed to the Tf-lite model
